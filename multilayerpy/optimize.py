...
class Optimizer():
    ...
    def __init__(self, simulate_object, cost='MSE',
                 cfunc=None, param_evolution_func=None,
                 param_evolution_func_extra_vary_params=None,
                 custom_model_y_func=None):
    ...
    def cost_func(self, model_y, weighted=True):
        ...
        '''
        # check if data has a time = 0 s
        ADD_T0 = False
        if float(self.data.x[0]) != 0.0:
            ADD_T0 = True

        # use user-supplied cost function if supplied
        if not isinstance(self.cfunc, type(None)):
            if not ADD_T0:
                val = self.cfunc(self.data, model_y)

            # warn the user that their data needs a t=0 datapoint
            # in the cost function
            #TODO carry on
            else:
                if len(self.data.x) != len(model_y):
                    raise RuntimeError('You need to add a t=0 datapoint to your experimental data')
                val = self.cfunc(self.data, model_y)
    ...
    def lnlike(self, vary_params):
        ...
        # TODO need to save original sim.parameters to go back to
        if not isinstance(vary_param_keys, type(None)):
        ...
        if sim.model.model_type.lower() == 'km-sub':
            ...
            if not isinstance(self.custom_model_y_func, type(None)):
                model_y = self.custom_model_y_func(bulk_concs, surf_concs,
                                                   V, A)
            else:
            ...
            if not isinstance(self._fitting_component_no, type(None)):
    ...
    def fit(self, weighted=True, method='least_squares',
        ...
        if len(add_param_names) != 0:
            ...
            for i in range(len(add_param_names)):
                ...
                optimised_param_obj = Parameter(result.x[i+len(varying_param_keys)],
                                                name=key, vary=True,
    ...
    def sample(self, samples, n_walkers=100, n_burn=0, pool=None, **kwargs):
        ...
        p0 = [np.array(varying_params) * np.random.normal(1.0, 1e-9,
                                                          len(varying_params)) for i in range(n_walkers)]
    ...
    def plot_chains(self, discard=0, thin=1):
        ...
        fig, ax = plt.subplots(nrows=n_rows,
                               ncols=3, figsize=(3*2.5, n_rows*2.5))
    ...
    def plot_chain_outputs(self):
        ...
        if self._sampled_xy_data is None:
        ...
        plt.errorbar(time, data_y, yerr=data_y_err, mfc='none',
                    mec='k', linestyle='none', label='data', marker='o',
                    color='k',
                    )
        ...
        for i, sample_output in enumerate(sampled_data):
            ...
            if i == 0:
                plt.plot(t,y/y[0], ls='-',
                         color=(1,0,0,0.1), label='sampled model output', lw=1)
            else:
    ...
    def get_chain_outputs(self, n_samples='all', parallel=False,
                          component_number=1, n_burn=0, thin=1,
        ...
        if self._sampled_xy_data is not None and not override_stop_run:
        ...
        if parallel:
            ...
            # TODO potential to fall over - requires more testing
            if __name__ == "__main__":

                with Pool() as p:
                    xy_array_output = p.map(self._get_xy_data_sample,
                                            sim_obj_list)

                    print('xy_array_output',
                          xy_array_output)
    ...
    def _get_xy_data_sample(self, sim):
        ...
        sim.run(sim.run_params['n_layers'], sim.run_params['rp'],
                sim.run_params['time_span'],
                sim.run_params['n_time'],
                sim.run_params['V'], sim.run_params['A'],
                sim.run_params['layer_thick'],
                sim.run_params['Y0'],
                ode_integrator=sim.run_params['ode_integrator'],
                ode_integrate_method=sim.run_params['ode_integrate_method'],
